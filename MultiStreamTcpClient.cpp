	  ...
	  for (int stream = 0; stream < socketNum; stream++)
	  {
		for (int i = 0; i < numMsgs; i++)
		{
		  ...
		  if (streams[stream] && msgSizes[stream] > 0)
		  {
			...
			int sent = streams[stream]->send(msgs[stream], msgSizes[stream]);
			//printf("Bytes sent %d out of %d\n", sent, msgSizes[stream]);
			//  }
			//}
			//}
			//
			...
			if (streams[stream] && msgSizes[stream] > 0)
			{
			  ...
			  //printf("Bytes read before loop %d out of %d\n", bytesRead, msgSizes[stream]);
