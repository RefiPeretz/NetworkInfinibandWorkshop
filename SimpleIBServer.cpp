...
int setupIB()
{
    ...
    connection = init_connection(ib_dev, size, rx_depth, ib_port, use_event, 1,
                                 peerNum, messageChar);
    ...
    {
        connection->routs =
                postRecvWorkReq(connection, (*connection).rx_depth, tidQP);
    ...
    if (connectRemoteToClient(connection, ib_port, mtu, port, sl, gidx,
                              _localQPinfo, _remoteQPinfo))
    {
        std::cerr
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        std::cerr << "Usage: <client threads num, Default is 1>" << std::endl;
        return 1;
    }
            ...
            for (i = 0; i < ne; ++i)
            {
                ...
                switch ((int) wc[i].wr_id)
                {
                    ...
                    case RECV_WRID:
                        ...
                        {
                            routs += postRecvWorkReq(connection,
                                                     connection->rx_depth -
                                                     routs, threadId);
                            if (routs < connection->rx_depth)
                            {
